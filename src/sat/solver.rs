...
impl SATSolver {
    ...
    pub fn add_clause(&mut self, literals: Vec<Literal>) {
        ...
        self.clause_theory.add_clause(&self.variable_manager, literals, false, &mut self.tentative_assigned_variable_queue);
    }
    ...
    fn search(&mut self, conflict_count_limit: usize) -> SearchResult {
        loop {
            ...
            if let PropagationResult::Conflict { variable_index, reasons } = propagation_result {
                ...
                // 学習節を追加
                self.clause_theory.add_clause(&self.variable_manager, learnt_clause, true, &mut self.tentative_assigned_variable_queue);
        ...
        loop {
            ...
            match self.variable_manager.get_state(variable_index) {
                ...
                VariableState::Unassigned { last_assigned_value } => {
                    ...
                    // 前に割り当てられていた値を割り当て
                    self.tentative_assigned_variable_queue.insert(variable_index, last_assigned_value, Reason::Decision);
                    break;
                }
            }
        }
    ...
    fn propagate(&mut self) -> PropagationResult {
        ...
        while !self.tentative_assigned_variable_queue.is_empty() {
            if self.tentative_assigned_variable_queue.has_conflict() {
                ...
                let (variable_index, reasons) = self.tentative_assigned_variable_queue.iter_conflicting_variables().next().unwrap();
                return PropagationResult::Conflict { variable_index: *variable_index, reasons: reasons.clone() };
            }
            // 伝播によって仮割り当てされた変数のうち最も優先度の高いものを取り出す
            let (variable_index, value, reason) = self.tentative_assigned_variable_queue.pop_first_consistent_variable().unwrap();
            ...
            // 割り当てを通知
            self.clause_theory.inform_assignment(&self.variable_manager, variable_index, &mut self.tentative_assigned_variable_queue);
        }
    ...
    fn resolve(&mut self, variable_index: usize, value: bool, reason: Reason) {
        ...
        for literal in self.literal_buffer.iter() {
            if self.analyzer_buffer.contains_key(literal.index) {
                ...
                if let VariableState::Assigned {decision_level, assignment_level, reason , .. } = self.variable_manager.get_state(literal.index) {
                    if decision_level != 0 {
                        self.analyzer_buffer.insert(literal.index, AnalyzerBufferValue {
                            sign: literal.sign,
    ...
    fn analyze(&mut self, conflicting_variable_index: usize, reasons: [Reason; 2]) -> (usize, Vec<Literal>) {
        ...
        loop {
            ...
            { // バックジャンプ可能かを判定
                ...
                let first_three_decision_levels = Vec::from_iter(
                    self.analyzer_buffer.iter()
                    .take(3)
                    .map(|item| item.1.decision_level)
                );
